...
`),b(t),[]}}function Cr(){A.clear();}async function qe(e,t){try{if(!await He())return null;e.includes("index")&&e.unshift("index");let i=await tt(e,t),n=await k(i),o=z$1.array(P).parse(n);if(!o)return null;if(e.includes("index")&&t.tailwind.baseColor){let p=await et(t.tailwind.baseColor,t);p&&o.unshift(p);}let l={};o.forEach(p=>{l=E(l,p.tailwind??{});});let g={};o.forEach(p=>{g=E(g,p.cssVars??{});});let y="";return o.forEach(p=>{p.docs&&(y+=`${p.docs}
`);}),G.parse({dependencies:E.all(o.map(p=>p.dependencies??[])),devDependencies:E.all(o.map(p=>p.devDependencies??[])),files:E.all(o.map(p=>p.files??[])),tailwind:l,cssVars:g,docs:y})}catch(r){return b(r),null}}async function Xe(e,t){let r=new Set,i=[],n=t.resolvedPaths?.cwd?await Z(t.resolvedPaths.cwd,t.style):t.style;async function o(l){let g=se(U(l)?l:`styles/${n}/${l}.json`);if(!r.has(g)){r.add(g);try{let[y]=await k([g]),p=P.parse(y);if(i.push(g),p.registryDependencies)for(let f of p.registryDependencies)await o(f);}catch(y){console.error(`Error fetching or parsing registry item at ${l}:`,y);}}}return await o(e),Array.from(new Set(i))}async function et(e,t){let[r,i]=await Promise.all([Qe(e),Y(t)]);if(!r)return null;let n={name:e,type:"registry:theme",tailwind:{config:{theme:{extend:{borderRadius:{lg:"var(--radius)",md:"calc(var(--radius) - 2px)",sm:"calc(var(--radius) - 4px)"},colors:{}}}}},cssVars:{light:{radius:"0.5rem"},dark:{}}};return t.tailwind.cssVariables&&(n.tailwind.config.theme.extend.colors={...n.tailwind.config.theme.extend.colors,...ne(r.cssVars.dark)},n.cssVars={light:{...r.cssVars.light,...n.cssVars.light},dark:{...r.cssVars.dark,...n.cssVars.dark}},i==="v4"&&r.cssVarsV4&&(n.cssVars={light:{...n.cssVars.light,...r.cssVarsV4.light},dark:{...n.cssVars.dark,...r.cssVarsV4.dark}})),n}function se(e){if(U(e)){let t=new URL(e);return t.pathname.match(/\/chat\/b\//)&&!t.pathname.endsWith("/json")&&(t.pathname=`${t.pathname}/json`),t.toString()}return `${Je}/${e}`}function U(e){try{return new URL(e),!0}catch{return !1}}async function tt(e,t){let r=[];for(let i of e){let n=await Xe(i,t);r.push(...n);}return Array.from(new Set(r))}function Lr(){return new Map([["registry:ui","ui"],["registry:lib","lib"],["registry:hook","hooks"],["registry:block","components"],["registry:component","components"]])}function Ir(e){let t=new Map;return e.forEach(r=>{r.registryDependencies&&r.registryDependencies.forEach(i=>{t.set(i,r);});}),t}

export { it as A, R as B, N as C, M as D, B as E, G as F, He as G, Pr as H, Tr as I, vr as J, Er as K, Qe as L, Ze as M, jr as N, Sr as O, k as P, Cr as Q, qe as R, et as S, tt as T, Lr as U, Ir as V, u as a, Ot as b, Vt as c, Ft as d, zt as e, H as f, j as g, V as h, Dt as i, Se as j, Le as k, $ as l, L as m, kt as n, Y as o, m as p, q, b as r, ar as s, ee as t, Ue as u, W as v, oe as w, ae as x, le as y, P as z };
//# sourceMappingURL=out.js.map
//# sourceMappingURL=chunk-OR35VCLR.js.map